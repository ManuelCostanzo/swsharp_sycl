...
static void *ovFindScoreGpuKernel(void *params);
...
extern void ovFindScoreGpu(int *queryStart, int *targetStart, Chain *query,
  ...
  if (thread == NULL) {
    ovFindScoreGpuKernel(param);
  } else {
    threadCreate(thread, ovFindScoreGpuKernel, (void *)param);
  }
}
...
static void solveShort(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
  ...
  if (groupId == (groupRangeId - 1)) {
    solveShortDelegated(d, vBus, hBus, sub, item_ct1, gapOpen_, gapExtend_,
                        rows_, cols_, realRows_, realCols_, cellWidth_, score_,
                        pLeft_, pRight_, scorerLen_, subLen_, match_, mismatch_,
                        res_, hBusScrShr, hBusAffShr, rowGpu, colGpu, subGpu);
  } else {
    solveShortNormal(d, vBus, hBus, sub, item_ct1, gapOpen_, gapExtend_, rows_,
                     cols_, realRows_, realCols_, cellWidth_, score_, pLeft_,
                     pRight_, scorerLen_, subLen_, match_, mismatch_, res_,
...
static void *ovFindScoreGpuKernel(void *params) try {
            ...
            .submit([&](sycl::handler &cgh) {
              ...
              cgh.parallel_for(
                  ...
                  [=](sycl::nd_item<1> item_ct1) {
                    solveShort(
                        diagonal, vBusGpu, hBus, SubScalar(), item_ct1, *ga,
                        *ge, rowsGpu, colsGpu, rows, cols, cellWidth, score,
                        pLeft, pRight, scorerLen, subLen, match, mismatch, res,
        ...
        dev_q
            .submit([&](sycl::handler &cgh) {
              ...
              cgh.parallel_for(
                  ...
                  [=](sycl::nd_item<1> item_ct1) {
                    solveShort(
                        diagonal, vBusGpu, hBus, SubScalarRev(), item_ct1, *ga,
                        *ge, rowsGpu, colsGpu, rows, cols, cellWidth, score,
                        pLeft, pRight, scorerLen, subLen, match, mismatch, res,
                        hBusScrShr_acc_ct1.get_pointer(),
                        hBusAffShr_acc_ct1.get_pointer(), rowGpu, colGpu, sub);
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<1> item_ct1) {
                  solveShort(diagonal, vBusGpu, hBus, SubVector(), item_ct1,
                             *ga, *ge, rowsGpu, colsGpu, rows, cols, cellWidth,
                             score, pLeft, pRight, scorerLen, subLen, match,
                             mismatch, res, hBusScrShr_acc_ct1.get_pointer(),
      ...
      dev_q
          .submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(
                sycl::nd_range<1>(blocks * threads, threads),
                [=](sycl::nd_item<1> item_ct1) {
                  solveLong(diagonal, vBusGpu, hBus, SubVector(), item_ct1, *ga,
                            *ge, rowsGpu, colsGpu, rows, cols, cellWidth, score,
                            pLeft, pRight, scorerLen, subLen, match, mismatch,
                            res, hBusScrShr_acc_ct1.get_pointer(),
                            hBusAffShr_acc_ct1.get_pointer(), rowGpu, colGpu,
                            sub);
                });
          })
