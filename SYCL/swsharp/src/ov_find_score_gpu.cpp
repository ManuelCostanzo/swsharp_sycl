                    ...
                    sycl::char4 *rowGpu, char *colGpu, int *subGpu)
{
    ...
    int x1 =
        cellWidth_ * (groupRangeId - groupId - 1) +
        localRangeId;
    int y1 = (d + groupId - groupRangeId + 1) *
             (localRangeId * 4);

    int x2 =
        cellWidth_ * (groupRangeId - groupId - 1) -
        localRangeId;
    int y2 = (d + groupId - groupRangeId + 2) *
             (localRangeId * 4);
    ...
    if (y1 - x1 > pLeft_ && (x2 - y2 > pRight_ || y2 < 0))
    {
        ...
        vBus.mch[(row >> 2) % (groupRangeId *
                               localRangeId)] = SCORE_MIN;
        vBus.scr[(row >> 2) % (groupRangeId *
                               localRangeId)] = SCORE4_MIN;
        vBus.aff[(row >> 2) % (groupRangeId *
                               localRangeId)] = SCORE4_MIN;
                       ...
                       sycl::char4 *rowGpu, char *colGpu, int *subGpu)
{
    ...
    {
        solveShortNormal(d, vBus, hBus, sub, item_ct1, gapOpen_, gapExtend_,
                         rows_, cols_, realRows_, realCols_, cellWidth_, score_,
                         pLeft_, pRight_, scorerLen_, subLen_, match_, mismatch_,
                         res_, hBusScrShr, hBusAffShr, rowGpu, colGpu, subGpu);
    }
    ...
    if (isValid)
    {
        ...
        if (isValid)
        {
            ...
            if (x2 - y2 > pRight_)
            {
                ...
                for (int i = 0; i < cellWidth_ - localRangeId;
                     i += localRangeId)
                {
    ...
    for (int diagonal = 0; diagonal < diagonals; ++diagonal)
    {
        if (scalar)
        {
          ...
          cgh.parallel_for(sycl::nd_range<1>(blocks*threads, threads),
                    ...
                    *ga, *ge, rowsGpu, colsGpu, rows, cols,
                    cellWidth, score, pLeft, pRight, scorerLen, subLen,
                    match, mismatch, res, hBusScrShr_acc_ct1.get_pointer(),
          ...
          cgh.parallel_for(sycl::nd_range<1>(blocks*threads, threads),
                       [=](sycl::nd_item<1> item_ct1) {
                            solveShort( diagonal, vBusGpu, hBus, SubScalarRev(), item_ct1,
                    *ga, *ge, rowsGpu, colsGpu, rows, cols,
                    cellWidth, score, pLeft, pRight, scorerLen, subLen,
                    match, mismatch, res, hBusScrShr_acc_ct1.get_pointer(),
                    hBusAffShr_acc_ct1.get_pointer(), rowGpu, colGpu, sub);
