                 ...
                 sycl::char4 *rowGpu, char *colGpu, int *subGpu)
{
    ...
    if (valid)
    {
        ...
        if (y1 - x1 > pLeft_)
        {
            ...
            const int id = sycl::min(row >> 2, (int)(rows_ / 4)) % (groupRangeId * localRangeId);
    ...
    for (int diagonal = 0; diagonal < diagonals; ++diagonal)
    {
        if (scalar)
        {
         ...
         cgh.parallel_for(sycl::nd_range<1>(blocks*threads, threads),
                ...
                solveShort(
                    ...
                    queryFrontGap, targetFrontGap, *ga, *ge, gapDiff,
                    dRow, rowsGpu, colsGpu, cellWidth, pLeft, pRight, scorerLen, subLen, match,
         ...
         cgh.parallel_for(sycl::nd_range<1>(blocks*threads, threads),
                ...
                solveLong(
                    diagonal, vBusGpu, hBus, SubScalar(), item_ct1,
                    queryFrontGap, targetFrontGap, *ga, *ge, gapDiff,
                    dRow, rowsGpu, colsGpu, cellWidth, pLeft, pRight,
         ...
         cgh.parallel_for(sycl::nd_range<1>(blocks*threads, threads),
                ...
                solveLong(
                    diagonal, vBusGpu, hBus, SubScalarRev(), item_ct1,
                    queryFrontGap, targetFrontGap, *ga, *ge, gapDiff, dRow, rowsGpu, colsGpu,
                    cellWidth, pLeft, pRight, scorerLen, subLen, match,
                    mismatch, hBusScrShr_acc_ct1.get_pointer(),
                    hBusAffShr_acc_ct1.get_pointer(), rowGpu, colGpu, sub);
              }); })
                    .wait();
        ...
        sycl::accessor<int, 1, sycl::access_mode::read_write,
         ...
         cgh.parallel_for(sycl::nd_range<1>(blocks*threads, threads),
                       [=](sycl::nd_item<1> item_ct1) {
                solveShort(
                    diagonal, vBusGpu, hBus, SubVector(), item_ct1,
                    queryFrontGap, targetFrontGap, *ga, *ge, gapDiff,
                    dRow, rowsGpu, colsGpu, cellWidth, pLeft, pRight,
                    scorerLen, subLen, match, mismatch, hBusScrShr_acc_ct1.get_pointer(),
                    hBusAffShr_acc_ct1.get_pointer(), rowGpu, colGpu, sub);
