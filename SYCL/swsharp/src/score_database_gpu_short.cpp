...
static void hwSolveShortGpu(int *scores, sycl::int2 *hBus, int *lengths,
                            ...
                            int width_, int qpWidth_, int *seqsGpu, sycl::char4 *qpGpu);
...
try
{
    ...
    sycl::int2 *hBusGpu = sycl::malloc_device<sycl::int2>(sequencesCols * hBusHeight, dev_q);
    dev_q.memset(hBusGpu, 0, sequencesCols * hBusHeight * sizeof(sycl::int2)).wait();
...
try
{
    ...
    while (1)
    {
        ...
        dev_q.submit([&](sycl::handler &cgh)
                     {
            if (type == SW_ALIGN) {
                cgh.parallel_for(sycl::nd_range<1>(max_blocks * max_threads, max_threads), [=](sycl::nd_item<1> item_ct1) {
                    swSolveShortGpu(scoresGpu, hBusGpu, lengthsGpu, lengthsPaddedGpu,
                                                                  offsetsGpu, indexesGpu, indexesLenLocal, block,
                                                                  item_ct1, gapOpen, gapExtend,
                                                                  rows, rowsGpu, sequencesCols, qpWidth, seqsGpu, qpGpu);   });
            }  else if (type == NW_ALIGN) {
                cgh.parallel_for(sycl::nd_range<1>(max_blocks * max_threads, max_threads),  [=](sycl::nd_item<1> item_ct1)  {
                    nwSolveShortGpu(scoresGpu, hBusGpu, lengthsGpu, lengthsPaddedGpu,
                                                                  offsetsGpu, indexesGpu, indexesLenLocal, block,
                                                                  item_ct1, gapOpen, gapExtend,
                                                                  rows, rowsGpu, sequencesCols, qpWidth, seqsGpu, qpGpu);});
            }  else if (type == HW_ALIGN) {
                cgh.parallel_for(sycl::nd_range<1>(max_blocks * max_threads, max_threads),  [=](sycl::nd_item<1> item_ct1)  {
                    hwSolveShortGpu(scoresGpu, hBusGpu, lengthsGpu, lengthsPaddedGpu,
                                                                  offsetsGpu, indexesGpu, indexesLenLocal, block,
                                                                  item_ct1, gapOpen, gapExtend,
                                                                  rows, rowsGpu, sequencesCols, qpWidth, seqsGpu, qpGpu);});
                ...
                cgh.parallel_for(sycl::nd_range<1>(max_blocks * max_threads, max_threads), [=](sycl::nd_item<1> item_ct1) { 
                    ovSolveShortGpu(scoresGpu, hBusGpu, lengthsGpu, lengthsPaddedGpu,
                                           offsetsGpu, indexesGpu, indexesLenLocal, block,
                                           item_ct1, gapOpen, gapExtend,
                                           rows, rowsGpu, sequencesCols, qpWidth, seqsGpu, qpGpu); });
    } })
...
static void
hwSolveShortGpu(int *scores, sycl::int2 *hBus, int *lengths,
                int *lengthsPadded, int *offsets, int *indexes,
                int indexesLen, int block,
                sycl::nd_item<1> item_ct1, int gapOpen_,
                int gapExtend_, int rows_, int rowsPadded_,
...
#pragma unroll
            for (int k = 0; k < 4; ++k)
            {
                ...
                sycl::char4 rowScores = qpGpu[(i >> 2) * qpWidth_ + sycl::min(static_cast<int>(code), qpWidth_ - 1)];
                ...
                rowScores = qpGpu[((i >> 2) + 1) * qpWidth_ + sycl::min(static_cast<int>(code), qpWidth_ - 1)];
...
static void
nwSolveShortGpu(int *scores, sycl::int2 *hBus, int *lengths,
                int *lengthsPadded, int *offsets, int *indexes,
                int indexesLen, int block,
                sycl::nd_item<1> item_ct1, int gapOpen_,
                int gapExtend_, int rows_, int rowsPadded_,
...
static void
ovSolveShortGpu(int *scores, sycl::int2 *hBus, int *lengths,
                int *lengthsPadded, int *offsets, int *indexes,
                int indexesLen, int block,
                sycl::nd_item<1> item_ct1, int gapOpen_,
                int gapExtend_, int rows_, int rowsPadded_,
...
static void
swSolveShortGpu(int *scores, sycl::int2 *hBus, int *lengths,
                int *lengthsPadded, int *offsets, int *indexes,
                int indexesLen, int block,
                sycl::nd_item<1> item_ct1, int gapOpen_,
                int gapExtend_, int rows_, int rowsPadded_,
