...
static ScorerConstants scorerConstants[] = {
    {"BLOSUM_62", -1, -1, 0.3176, 0.134, 0.4012, 0.7916, 0.623757, 4.964660,
     4.964660, 0},
    {"BLOSUM_62", 11, 2, 0.297, 0.082, 0.27, 1.1, 0.641766, 12.673800,
     12.757600, 0},
    {"BLOSUM_62", 10, 2, 0.291, 0.075, 0.23, 1.3, 0.649362, 16.474000,
     16.602600, 0},
    ...
    {"BLOSUM_62", 7, 2, 0.239, 0.027, 0.10, 2.5, 0.702056, 61.238300, 61.886000,
     0},
    {"BLOSUM_62", 6, 2, 0.201, 0.012, 0.061, 3.3, 0.740802, 140.417000,
     141.882000, 0},
    {"BLOSUM_62", 13, 1, 0.292, 0.071, 0.23, 1.2, 0.647715, 19.506300,
     19.893100, 0},
    {"BLOSUM_62", 12, 1, 0.283, 0.059, 0.19, 1.5, 0.656391, 27.856200,
     28.469900, 0},
    {"BLOSUM_62", 11, 1, 0.267, 0.041, 0.14, 1.9, 0.669720, 42.602800,
     43.636200, 0},
    {"BLOSUM_62", 10, 1, 0.243, 0.024, 0.10, 2.5, 0.693267, 83.178700,
     85.065600, 0},
    {"BLOSUM_62", 9, 1, 0.206, 0.010, 0.052, 4.0, 0.731887, 210.333000,
     214.842000, 0},
...
extern EValueParams *createEValueParams(long long length, Scorer *scorer) {
  ...
  for (int i = 0; i < (int)SCORER_CONSTANTS_LEN; ++i) {
    ...
    if (sameName && entry->gapOpen == gapOpen &&
        entry->gapExtend == gapExtend) {
      index = i;
      break;
    }
  }
...
extern void eValues(double *values, int *scores, Chain *query, Chain **database,
...
static void eValuesGpu(double *values, int *scores, Chain *query,
  ...
  sycl::queue dev_q = queues[cards[0]];
        ...
        .submit([&](sycl::handler &cgh) {
          cgh.parallel_for(sycl::nd_range<1>(blocks * threads, threads),
                           [=](sycl::nd_item<1> item_ct1) {
                             kernelDna(valuesGpu, dataGpu, item_ct1, length,
                                       queryLen, paramsLambda, paramsLogK);
                           });
        })
        .wait();
  } else {
    ...
    dev_q
        .submit([&](sycl::handler &cgh) {
          cgh.parallel_for(
              ...
              [=](sycl::nd_item<1> item_ct1) {
                kernelProt(valuesGpu, dataGpu, item_ct1, databaseLen, queryLen,
                           length, paramsLambda, paramsK, paramsA, paramsB,
                           paramsAlpha, paramsBeta, paramsSigma, paramsTau);
              });
        })
        .wait();
  }
