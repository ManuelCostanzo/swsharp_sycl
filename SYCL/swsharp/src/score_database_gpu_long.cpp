          ...
          cgh.parallel_for(
              ...
              [=](sycl::nd_item<1> item_ct1) {
                swSolve(codesGpu, startsGpu, lengthsGpu, indexesGpu, scoresGpu,
                        hBusGpu, item_ct1, gapOpen, gapExtend, rows, rowsGpu,
                        indexesLen, iters, scoresShr_acc_ct1.get_pointer(),
...
void hwSolveSingle(int id, char *codes, int *starts, int *lengths, int *scores,
  ...
  for (int i = 0; i < width; ++i) {
    ...
    if (valid) {
      if (iter < iters_ - 1 && localId == localRangeId - 1) {
