                             ...
                             int scores[], bool calculated[], const int overflowMethod)
{
    ...
    while (numEndedDbSeqs < dbLength)
    {
        ...
        if (overflowDetected || shortestDbSeqLength == columnsSinceLastSeqEnd)
        { // If at least one sequence ended
            ...
            for (int i = 0; i < SIMD::numSeqs; i++)
            {
                ...
                { // If not null sequence
                    ...
                    if (overflowed[i] || currDbSeqsLengths[i] == 0)
                    { // If sequence ended
                        numEndedDbSeqs++;
                        if (!overflowed[i])
                        {
                                    ...
                                    int &numEndedDbSeqs)
{
    ...
    if (nextDbSeqIdx < dbLength)
    { // If there is sequence to load
    ...
    {                                        // If there are no more sequences to load, load "null" sequence
                          ...
                          int scores[], const int overflowMethod)
{
    ...
    for (int startIdx = 0; startIdx < dbLength; startIdx += chunkSize)
    {
        ...
        for (int i = 0; i < dbLength_; i++)
            calculated[i] = false;
        resultCode = searchDatabase_<Simd<char>, MODE>(query, queryLength, db_, dbLength_, dbSeqLengths_,
                                                       gapOpen, gapExt, scoreMatrix, alphabetLength, scores_,
                                                       calculated, overflowMethod);
        if (resultCode == SWIMD_ERR_OVERFLOW)
        {
            resultCode = searchDatabase_<Simd<short>, MODE>(query, queryLength, db_, dbLength_, dbSeqLengths_,
                                                            gapOpen, gapExt, scoreMatrix, alphabetLength, scores_,
                                                            calculated, overflowMethod);
            if (resultCode == SWIMD_ERR_OVERFLOW)
            {
                resultCode = searchDatabase_<Simd<int>, MODE>(query, queryLength, db_, dbLength_, dbSeqLengths_,
                                                              gapOpen, gapExt, scoreMatrix, alphabetLength, scores_,
                                                              calculated, overflowMethod);
                if (resultCode != 0)
