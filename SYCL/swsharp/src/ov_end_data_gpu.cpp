...
solveShortDelegated(int d, VBus vBus, sycl::int2 *hBus, sycl::int3 *results,
  ...
  if (col == 0) {
    int rowPrev = row - groupRangeId * localRangeId * 4;
...
static void solveShort(int d, VBus vBus, sycl::int2 *hBus, sycl::int3 *results,
  ...
  if (groupId == (groupRangeId - 1)) {
    ...
    solveShortNormal(d, vBus, hBus, results, sub, item_ct1, gapOpen_,
                     gapExtend_, rows_, cols_, cellWidth_, scorerLen_, subLen_,
                     match_, mismatch_, hBusScrShr, hBusAffShr, rowGpu, colGpu,
                     subGpu);
  }
}
...
static void *ovEndDataGpuKernel(void *params) try {
    ...
    if (scalar) {
      ...
      dev_q
          .submit([&](sycl::handler &cgh) {
            ...
            cgh.parallel_for(sycl::nd_range<1>(blocks * threads, threads),
                             [=](sycl::nd_item<1> item_ct1) {
                               solveLong(diagonal, vBusGpu, hBus, results,
                                         SubScalarRev(), item_ct1, *ga, *ge,
                                         rowsGpu, colsGpu, cellWidth, scorerLen,
