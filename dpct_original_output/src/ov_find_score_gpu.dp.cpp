...
static dpct::constant_memory<int, 0> realRows_;
static dpct::constant_memory<int, 0> realCols_;
...
/*
DPCT1059:193: SYCL only supports 4-channel image format. Adjust the code.
*/
dpct::image_wrapper<char, 1> colTexture;
/*
DPCT1059:194: SYCL only supports 4-channel image format. Adjust the code.
*/
dpct::image_wrapper<sycl::int2, 1> hBusTexture;
/*
DPCT1059:195: SYCL only supports 4-channel image format. Adjust the code.
*/
...
bool valid(int row, int col, int realRows_, int realCols_);

static int gap(int idx, int gapOpen_, int gapExtend_);

template <class Sub>
static void
solveShortDelegated(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
                    sycl::nd_item<3> item_ct1, int gapOpen_, int gapExtend_,
                    int realRows_, int realCols_, int subLen_, int *hBusScrShr,
                    int *hBusAffShr,
...
solveShortNormal(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
                 sycl::nd_item<3> item_ct1, int realRows_, int realCols_,
...
static void solveShort(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
                       sycl::nd_item<3> item_ct1, int gapOpen_, int gapExtend_,
                       int realRows_, int realCols_, int subLen_,
...
solveShortDelegated(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
    ...
    if (0 <= row && row < rows_ && col > 0) {
        ...
        atom.mch = gap(row - 1, gapOpen_, gapExtend_) * (row != 0);
    ...
    for (int i = 0; i < item_ct1.get_local_range(2); ++i) {
        ...
        if (0 <= row && row < rows_) {
            ...
            /*
            DPCT1084:198: The function call has multiple migration results in
            ...
            */
            ...
            /*
            DPCT1084:199: The function call has multiple migration results in
            different template instantiations that could not be unified. You may
            need to adjust the code.
            */
            ...
            /*
            DPCT1084:200: The function call has multiple migration results in
            different template instantiations that could not be unified. You may
            need to adjust the code.
            */
            ...
            /*
            DPCT1084:201: The function call has multiple migration results in
            different template instantiations that could not be unified. You may
            need to adjust the code.
            */
...
solveShortNormal(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
    ...
    for (int i = 0; i < item_ct1.get_local_range(2); ++i, ++col) {
        ...
        int ins = sycl::max((int)(atom.lScr.x() - gapOpen_),
                            (int)(atom.lAff.x() - gapExtend_));
        /*
        DPCT1084:204: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        int mch = atom.mch + sub(columnCode, rowCodes.x(), subLen_, subTexture);
        ...
        ins = sycl::max((int)(atom.lScr.y() - gapOpen_),
                        (int)(atom.lAff.y() - gapExtend_));
        /*
        DPCT1084:205: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        mch = atom.lScr.x() + sub(columnCode, rowCodes.y(), subLen_, subTexture);
        ...
        ins = sycl::max((int)(atom.lScr.z() - gapOpen_),
                        (int)(atom.lAff.z() - gapExtend_));
        /*
        DPCT1084:206: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        mch = atom.lScr.y() + sub(columnCode, rowCodes.z(), subLen_, subTexture);
        ...
        ins = sycl::max((int)(atom.lScr.w() - gapOpen_),
                        (int)(atom.lAff.w() - gapExtend_));
        /*
        DPCT1084:207: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        mch = atom.lScr.z() + sub(columnCode, rowCodes.w(), subLen_, subTexture);
        ...
        if (atom.rScr.x() == score_ && valid(row, col, realRows_, realCols_)) {
         res_.x() = row; res_.y() = col;
        } else if (atom.rScr.y() == score_ &&
                   valid(row + 1, col, realRows_, realCols_)) {
         res_.x() = row + 1; res_.y() = col;
...
static void solveShort(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
    ...
    if (item_ct1.get_group(2) == (item_ct1.get_group_range(2) - 1)) {
        solveShortDelegated(d, vBus, hBus, sub, item_ct1, gapOpen_, gapExtend_,
                            realRows_, realCols_, subLen_, hBusScrShr,
                            hBusAffShr, subTexture, rowTexture, colTexture,
                            hBusTexture);
    } else {
        solveShortNormal(d, vBus, hBus, sub, item_ct1, realRows_, realCols_,
                         subLen_, hBusScrShr, hBusAffShr, subTexture,
...
static void solveLong(int d, VBus vBus, sycl::int2 *hBus, Sub sub,
    ...
    for (int i = 0; i < cellWidth_ - item_ct1.get_local_range(2); ++i, ++col) {
        ...
        int ins = sycl::max((int)(atom.lScr.x() - gapOpen_),
                            (int)(atom.lAff.x() - gapExtend_));
        /*
        DPCT1084:210: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        int mch = atom.mch + sub(columnCode, rowCodes.x(), subLen_, subTexture);
        ...
        ins = sycl::max((int)(atom.lScr.y() - gapOpen_),
                        (int)(atom.lAff.y() - gapExtend_));
        /*
        DPCT1084:211: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        mch = atom.lScr.x() + sub(columnCode, rowCodes.y(), subLen_, subTexture);
        ...
        ins = sycl::max((int)(atom.lScr.z() - gapOpen_),
                        (int)(atom.lAff.z() - gapExtend_));
        /*
        DPCT1084:212: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        mch = atom.lScr.y() + sub(columnCode, rowCodes.z(), subLen_, subTexture);
        ...
        ins = sycl::max((int)(atom.lScr.w() - gapOpen_),
                        (int)(atom.lAff.w() - gapExtend_));
        /*
        DPCT1084:213: The function call has multiple migration results in
        different template instantiations that could not be unified. You may
        need to adjust the code.
        */
        mch = atom.lScr.z() + sub(columnCode, rowCodes.w(), subLen_, subTexture);
...
static void *kernel(void *params) try {
    ...
    if (currentCard != card) {
        ...
        /*
        DPCT1093:214: The "card" may not be the best XPU device. Adjust the
        selected device if needed.
        */
        /*
        DPCT1003:215: Migrated API does not return error code. (*, 0) is
    ...
    int blocks;
    /*
    DPCT1005:216: The SYCL device version is different from CUDA Compute
    ...
    */
    ...
    sycl::char4 *rowGpu;
    /*
    DPCT1003:217: Migrated API does not return error code. (*, 0) is inserted.
    ...
    */
    CUDA_SAFE_CALL((rowGpu = (sycl::char4 *)sycl::malloc_device(
                    ...
                    0));
    /*
    DPCT1003:218: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL(
    ...
    /*
    DPCT1003:219: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((hBusGpu = (sycl::int2 *)sycl::malloc_device(
                    ...
                    0));
    /*
    DPCT1003:220: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL(
        (dpct::get_default_queue().memcpy(hBusGpu, hBusCpu, hBusSize).wait(),
         0));
    /*
    DPCT1003:221: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    ...
    */
    CUDA_SAFE_CALL((
        ...
        0));
    /*
    DPCT1003:226: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL(
    ...
    */
    ...
    CUDA_SAFE_CALL((dpct::get_default_queue()
                    ...
                    0));
    CUDA_SAFE_CALL((dpct::get_default_queue()
                        .memcpy(realRows_.get_ptr(), &rows, sizeof(int))
                        .wait(),
                    0));
    CUDA_SAFE_CALL((dpct::get_default_queue()
                        .memcpy(realCols_.get_ptr(), &cols, sizeof(int))
                        .wait(),
                    0));
    CUDA_SAFE_CALL((dpct::get_default_queue()
    ...
    for (int diagonal = 0; diagonal < diagonals; ++diagonal) {
        ...
        if (scalar) {
            if (subCpu[0] >= subCpu[1]) {
                /*
                DPCT1049:229: The work-group size passed to the SYCL kernel may
                ...
                */
                dpct::get_default_queue().submit([&](sycl::handler &cgh) {
                    ...
                    auto realRows__ptr_ct1 = realRows_.get_ptr();
                    auto realCols__ptr_ct1 = realCols_.get_ptr();
                    ...
                    cgh.parallel_for(
                        ...
                        [=](sycl::nd_item<3> item_ct1) {
                            solveShort(diagonal, vBusGpu, hBusGpu, SubScalar(),
                                       item_ct1, *gapOpen__ptr_ct1,
                                       *gapExtend__ptr_ct1, *realRows__ptr_ct1,
                                       *realCols__ptr_ct1, *subLen__ptr_ct1,
                    ...
                    cgh.parallel_for(
                        ...
                        [=](sycl::nd_item<3> item_ct1) {
                            solveShort(diagonal, vBusGpu, hBusGpu,
                                       ...
                                       *realRows__ptr_ct1, *realCols__ptr_ct1,
                                       *subLen__ptr_ct1,
                                       hBusScrShr_acc_ct1.get_pointer(),
                                       hBusAffShr_acc_ct1.get_pointer(),
                                       dpct::image_accessor_ext<int, 1>(
            ...
            /*
            DPCT1049:231: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
        ...
        /*
        DPCT1003:232: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        CUDA_SAFE_CALL((dpct::get_default_queue()
        ...
        if (scalar) {
            if (subCpu[0] >= subCpu[1]) {
                /*
                DPCT1049:233: The work-group size passed to the SYCL kernel may
                exceed the limit. To get the device limit, query
                info::device::max_work_group_size. Adjust the work-group size if
                needed.
                */
                ...
                /*
                DPCT1049:234: The work-group size passed to the SYCL kernel may
                exceed the limit. To get the device limit, query
                info::device::max_work_group_size. Adjust the work-group size if
                needed.
                */
            ...
            /*
            DPCT1049:235: The work-group size passed to the SYCL kernel may
            exceed the limit. To get the device limit, query
            info::device::max_work_group_size. Adjust the work-group size if
            needed.
            */
        ...
        /*
        DPCT1003:236: Migrated API does not return error code. (*, 0) is
        inserted. You may need to rewrite this code.
        */
        CUDA_SAFE_CALL((dpct::get_default_queue()
                            .memcpy(&res, res_.get_ptr(), sizeof(sycl::int2))
    ...
    *targetStart = res.y();
    ...
    /*
    DPCT1003:237: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    ...
    /*
    DPCT1003:238: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((colTexture.detach(), 0));
    /*
    DPCT1003:239: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
    CUDA_SAFE_CALL((hBusTexture.detach(), 0));
    /*
    DPCT1003:240: Migrated API does not return error code. (*, 0) is inserted.
    You may need to rewrite this code.
    */
